...
class BasicAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        """ Factors need modification """
        self.foodFactor = pow(self.getFoodNum(gameState), 0.5) / 2  # TODO food carry factor
...
class OffensiveAgent(BasicAgent):
    def chooseAction(self, gameState):
        self.updateEnemyPred(gameState)
        # print(self.enemyPred)
        # self.debugClear()
        # self.debugDraw([x.getPosition() for x in self.enemyPred if x is not None], [110, 110, 110])
        ...
        if self.isAtHome(pos, gameState):
            ...
            if len(self.getFoodList(gameState)) < 3:
                if self.getCarryingNum(gameState) == 0:
                    ...
                    heuristic = lambda x, y: self.searchHomeHeuristic(x, y, homeList, carryNum, enemyList)
        ...
        if len(actions) == 0:
            ...
            actions = [random.choice(legalActions)]

        action = actions[0]
        v = Actions.directionToVector(action)
        nextState = (pos[0] + int(v[0]), pos[1] + int(v[1]))
        # self.debugClear()
        # self.debugDraw([nextState], [110, 110, 110])

        nextDist = self.getEnemyDistance(nextState, enemyList)
        # print(nextDist)
        if len(nextDist) > 0:
            if nextDist[0] == 1:
                print("*********** STOP ************")
                ...
                print("********** ESCAPE ***********")
                newActions = [a for a in gameState.getLegalActions(self.index) if a != Directions.STOP and a != action]
                if len(newActions) == 0:
                    return Directions.STOP
                else:
                    return newActions[0]
    ...
    def searchFoodHeuristic(self, state, problem, foodList, homeList, carryNum, capsuleList):
        ...
        heuristic = self.getHomeWeight(state, homeList, carryNum)
        heuristic += self.getFoodWeight(state, foodList)
        heuristic += self.getCapsuleWeight(state, capsuleList) / 2  # TODO weight factor
        return self.heuristicWithMate(heuristic, gameState, state)

    def searchHomeHeuristic(self, state, problem, homeList, carryNum, enemyList):
        gameState = problem.getGameState()
        heuristic = self.getHomeWeight(state, homeList, carryNum)
        heuristic += self.getEnemyWeight(state, enemyList)
        return self.heuristicWithMate(heuristic, gameState, state)
...
class DefensiveAgent(OffensiveAgent):
    def chooseAction(self, gameState):
        ...
        self.debugClear()
        self.debugDraw([self.enemyHistory[-1][0][0], self.enemyHistory[-1][1][0]], [100, 125, 150])
        ...
        if self.isAtHome(gameState.getAgentPosition(self.index), gameState):
            if self.getScore(gameState) + gameState.getAgentState(self.teammate).numCarrying > 0: #self.foodNum / 5) and enemyavg > 3:
        ...
        if self.gridSafeMap[enemy[0]] * 2 + 1 >= self.getMazeDistance(gameState.getAgentPosition(self.index), enemy[0]) and not self.teamIsPanman(gameState, self.nearestLive(gameState, enemy[0])):
            ...
            dist1 = [self.getMazeDistance(food, enemy[0]) for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState)]
            dist1.sort()
            goal1 = [self.nearestLive(gameState, food) for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState) if self.getMazeDistance(food, enemy[0]) == dist1[0]]
            goal = goal1
            if self.choice == [False, False]:
                dist2 = [self.getMazeDistance(food, enemy_teammate[0]) for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState)]
                dist2.sort()
                goal2 = [self.nearestLive(gameState, food) for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState) if self.getMazeDistance(food, enemy_teammate[0]) == dist2[0]]
                ...
                if state1 == state2:
                    if self.getMazeDistance(gameState.getAgentPosition(self.index), state1) < self.getMazeDistance(gameState.getAgentPosition(self.teammate), state1) or (self.getMazeDistance(gameState.getAgentPosition(self.index), state1) == self.getMazeDistance(gameState.getAgentPosition(self.teammate), state1) and self.index < self.teammate):
                        if action1:
                            return action1[0]
                        else:
                            return Directions.STOP
                        ...
                        i = 1
                        while state1 == state2 and i < len(dist2):
                            goal2 = [self.nearestLive(gameState, food) for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState) if self.getMazeDistance(food, enemy[0]) == dist1[i]]
                            problem2 = PacmanContestProblem(self, gameState, goal2)
                            state2, action2 = aStarSearch2(self, gameState, problem2, pacmanContestHeuristic)
                            i += 1
                        if action2:
                    ...
                    goal = goal1
                    action = action1
                    self.debugDraw(goal, [125, 150, 100], False)
                else:
                    goal = goal2
                    action = action2
                    self.debugDraw(goal, [125, 150, 100], False)
